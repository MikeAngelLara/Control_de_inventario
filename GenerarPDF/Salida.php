<?php	
	...
	$mihtml .= '<th>Responsable</th>';
    $mihtml .= '<th>Fecha</th>';
	$mihtml .= '</tr>';
	$mihtml .= '</thead>';
	$mihtml .= '<tbody>';
	
	$resultado_transacciones = "SELECT * FROM salida";
	...
	while($transacciones = mysqli_fetch_assoc($resultado_transacciones)){
		...
		$mihtml .= '<td>'.$transacciones['responsable'] . "</td>";
	...
	// Cargar el HTML
	$dompdf->load_html('<h1 style="text-align: center;">Salida</h1>'. $mihtml .'
		');
