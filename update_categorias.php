...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
  ...
  if (empty($nombre_err) && empty($codigo_err)) {
    ...
    $sql = "UPDATE categorias SET nombre = ?, codigo = ? WHERE id = ?";

    if ($stmt = mysqli_prepare($link, $sql)) {
      # Bind variables to the prepared statement as parameters
      mysqli_stmt_bind_param($stmt, "ssi", $param_nombre, $param_codigo, $param_id);
      ...
      <div class="col-lg-6" style="margin-top: 23%;">
        ...
        <form action="<?= htmlspecialchars(basename($_SERVER["REQUEST_URI"]));  ?>" class="bg-light p-4 border" method="post" novalidate>
          <div class="row gy-3">
            ...
            <a class="regresar" href="categorias.php"><b>Cancelar  ⬅</b></a>
